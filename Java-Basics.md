...
+ 封装：将类的属性、方法等实现细节隐藏在类内部，仅对外提供部分允许访问的方法，使用者不必了解具体的实现细节。这样可以提高数据安全性，防止数据被恶意破坏。通常是将成员变量访问修饰设为private,并提供共有的setter和getter方法。
...
| getDec laredConstructors()                                   | 获取当前类的所有构造方法                           |
